...
ssh -i "%KEY%" ubuntu@161.118.191.181 "bash -c 'cd /home/ubuntu/hereiam_portal/hereiam && git fetch --all && git reset --hard origin/master && sudo pkill gunicorn || true && /home/ubuntu/hereiam_portal/venv/bin/gunicorn --workers 3 --bind 127.0.0.1:8000 \"app:create_app()\" --daemon && sudo systemctl restart nginx'"
